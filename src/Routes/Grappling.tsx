...
import { grapplingPageRequest } from "services/store-service";
...
const Grappling = () => {
  ...
  const { data: res, isSuccess } = useQuery("get-pagee-products", () => grapplingPageRequest({
    pageNo: currentPage,
    pageSize: 10,
    sortBy: sortByPage,
    sortDir: sortDirPage,
  ...
  const handlePageChange = useCallback(async (newPage: number) => {
    ...
    const newPageData = await grapplingPageRequest({
      pageNo: newPage,
      pageSize: 15,
      sortBy: sortByPage,
      sortDir: sortDirPage,
